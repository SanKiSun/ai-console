...
<script>
import { listFaq, getFaq, delFaq, addFaq, updateFaq } from "@/api/ai/faq";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除FAQ管理编号为"' + ids + '"的数据项？').then(function() {
        return delFaq(ids);
      }).then(() => {
        this.getList();
    ...
    /** 下载模板操作 */
    importTemplate() {
      this.download('ai/faq/importTemplate', {

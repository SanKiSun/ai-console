...
<script>
import "@ai/chatsdk/assets";
import { ChatSDK } from "@ai/chatsdk";
import Vue from "vue";
  ...
  mounted() {
    ...
    this.app = new ChatSDK({
      ...
      chat: {
        ...
        selectedToolList: [
          {
            icon: "aichat-iconfont aichat-xinhuihua",
            label: "分析需求",
            isEnable(selects) {
              console.log("isEnable trigger", selects);
              return selects.length > 1;
            },
            handler(selects, app, close) {
              console.log("当前选中的聊天记录", selects);
              app.setTaleList({
                componentName: "itemTalk",
                mine: true,
                value: {
                  text: "这是我自己添加的内容",
                },
        ...
        // 默认的内置功能
        featureOption: {
          // 切换悬浮或滑入 切换时会触发 affixChanged 事件
          // 注意 当前只在 sidebar 启用的情况下生效
          affix: true,
          // 提示词编辑
          promptEdit: {
            enable: true,
            // icon: "aichat-iconfont aichat-zhilingzhongxin",
            text: "AI提示词编写",
          },
          // 指令
          chatCommand: {
            ...
            top: [
              {
                ...
                component: {
                  ...
                  render() {
                    ...
                    return <div><h1>Hello world。{count}</h1></div>
                  }
                },
                handler() {
                  console.log("click");
            ...
            children: [
              {
                name: "新建页面",
                description:
                  "新建一个登录的页面，要求有账户密码的输入，和登录的按钮",
                id: "createPages",
                commandId: "gen_code_page_js_001",
                content: null,
              },
            ],
          },
